...
## Intégration comptage passagers (tx2)

Le comptage passagers n'utilise pas CVEDIA — il est assuré par **tx2**, un service FastAPI (YOLOX-nano + tracker) hébergé sur un Jetson TX2, qui détecte les franchissements de ligne caméra par caméra et gère sa propre base Postgres (instances, événements, historique d'état train).

- Le backend expose un proxy fin sous `/api/comptage-passagers/*` (voir `index.js`) vers tx2, configuré via la variable d'environnement `TX2_API_URL` (défaut `http://localhost:8000`).
- La page `/comptage-passagers/vue-ensemble` consomme ce proxy : KPIs (total/montées/descentes/caméras actives), bandeau d'état train (règle FR-001 : comptage actif uniquement si `in_station && cab_active`), et tableau des points de comptage avec démarrage/arrêt et flux vidéo en direct.
- Le déclenchement automatique (démarrage/arrêt selon l'état réel du train) est assuré par `bridge.py` (voir le dossier `bridge/` du dépôt racine), qui lit les données TCMS/AVMS du simulateur CDGX et pilote tx2 en conséquence.
- Les autres vues (`Analyse par caméra`, `Groupes`, `Événements`, `Rapports & export`) restent à connecter à ces mêmes données tx2.

## Ce qui reste à implémenter

**Intégration MDR6**
Le serveur MDR6 remonte les données de présence et d'état des trains. Son IP est supervisée par le pinger (disponibilité réseau uniquement), mais l'intégration applicative — récupération des données trains, modélisation en base, exposition via API et visualisation dans le dashboard — reste à construire. Le protocole exposé par le MDR6 doit être identifié en premier lieu (voir le document d'architecture de l'intégration tx2 pour l'état des lieux CIP/AVMS déjà observé sur le simulateur).
